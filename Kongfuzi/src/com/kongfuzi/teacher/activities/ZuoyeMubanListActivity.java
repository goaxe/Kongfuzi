...
public class ZuoyeMubanListActivity extends Activity {
  ...
  private Integer mCurrentTag = null;
  private String mCurrentClassBus = null;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mActivity = this;
    mCurrentTag = getIntent().getIntExtra(Constants.TAG, 0);
    mCurrentClassBus = getIntent().getStringExtra(Constants.CLASS_BUS);
    if (mCurrentClassBus == null) {
      Log.e(DEBUG_TAG, "class bus is null");
    } else  {
      Log.e(DEBUG_TAG, "class bus is not null");
    }
    ...
    public View getView(int arg0, View arg1, ViewGroup arg2) {
      ...
      view.setOnClickListener(new OnClickListener() {
        ...
        public void onClick(View v) {
          // TODO Auto-generated method stub
          if (mCurrentClassBus.equals(Constants.ZUOYE_MODIFY_ACTIVITY)) {
            ZuoyeModifyActivity.mBus.post(new RefreshPingyuEvent(mCurrentTag, item.content));
          } else if (mCurrentClassBus.equals(Constants.ZUOYE_SUBMIT_ACTIVITY)) {
            ZuoyeSubmitActivity.mBus.post(new RefreshPingyuEvent(mCurrentTag, item.content));
          } else if (mCurrentClassBus.equals(Constants.EXAM_SUBMIT_ACTIVITY)) {
            ExamSubmitActivity.mBus.post(new RefreshPingyuEvent(mCurrentTag, item.content));
          }
          finish();
        }
      });
          ...
          DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
              if (which == 0) {
                ...
                new AlertDialog.Builder(mActivity).setTitle("请输入模板内容").setView(editText)
                ...
                new AlertDialog.Builder(mActivity).setTitle("请输入评语").setView(editText)
                    .setPositiveButton("确定", listener).setNegativeButton("取消", null).show();
              }
              dialog.dismiss();
            }
  ...
  public class RefreshDataThread extends Thread {
    ...
    public void run() {
      ...
      try {
        JSONObject jsonObject = client.mubanList("zuoye", 0, 20);
